			...
			<ul class="nav flex-column mb-2">
				{#if savedPages.length === 0}
					<ul class="nav flex-column mb-0">
						<li class="nav-item">
							<p class="pr-4 pl-4">You do not have any saved pages.</p>
						</li>
					</ul>
				{:else}
			...
			<ul class="nav flex-column">
				...
				{#if rank.hasPermission("CreateAsset")}
					<li class="nav-item ml-4">
						<a use:link class="nav-link" href="/admin/asset/create"><FilePlusIcon /> Create Asset</a>
					</li>
				{/if}
				{#if rank.hasPermission("CreateAssetCopiedFromRoblox")}
					<li class="nav-item ml-4">
						<a use:link class="nav-link" href="/admin/asset/copy"><CopyIcon /> Copy Asset from Roblox</a>
					</li>
				{/if}
				{#if rank.hasPermission("SetAssetProduct")}
					<li class="nav-item ml-4">
						<a use:link class="nav-link" href="/admin/product/update"><EditIcon /> Update Asset</a>
					</li>
				{/if}
				{#if rank.hasPermission("MigrateAssetFromRoblox")}
					<li class="nav-item ml-4">
						<a use:link class="nav-link" href="/admin/asset/create-for-item"><FolderPlusIcon /> Create Asset</a>
					</li>
				{/if}
				{#if rank.hasPermission("CreateAssetVersion")}
					<li class="nav-item ml-4">
						<a use:link class="nav-link" href="/admin/asset/version/create"><UploadCloudIcon /> Update Asset File</a>
					</li>
				{/if}
				...
				{#if rank.hasPermission("RequestAssetReRender")}
					<li class="nav-item ml-4">
						<a use:link class="nav-link" href="/admin/asset/re-render"><RefreshCcwIcon /> Re-render Asset</a>
					</li>
				{/if}
				<li class="nav-item mt-2 d-md-none d-block">
					<a class="nav-link" href="/">Go back to Chronos</a>
				</li>
			</ul>
		</div>
	</nav>
