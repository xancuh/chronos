...
const UrlUtilities = (() => {
  const getDomainFromUrl = (url) => {
    ...
    if (baseDomainParsed.type === ParseResultType.Listed) {
      ...
      if (baseDomainParsed.hostname === 'yoblox.vip') {
        return 'yoblox.vip';
      }
      throw new Error('The only allowed reserved domain type is yoblox.vip, got ' + baseDomainParsed.hostname);
    } else {
    ...
    for (const item of Object.getOwnPropertyNames(result.headers)) {
      ...
      if (item === 'set-cookie') {
        ...
        if (typeof value === 'string') {
          value = value.replace(/roblox\.com/g, 'chrns.vip');
        } else {
          value.forEach((v, i, arr) => {
            arr[i] = v.replace(/roblox\.com/g, 'chrns.vip');
          });
        }
      }
