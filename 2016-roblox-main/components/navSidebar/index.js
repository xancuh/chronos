...
const staffIds = ['3', '27', '45', '6', '38', '91', '30', '10', '37', '9', '1'];
const isStaff = staffIds.includes(authStore.userId);
  ...
  return <div className={s.container}>
    <div className={s.card} style={{ paddingTop: paddingTop }}>
      ...
      <LinkEntry name='Updates' url='/info/blog' icon='icon-nav-blog' />
      <LinkEntry name='Users Online' url='/search/users?keyword=%20' icon='icon-nav-friends' />
      <LinkEntry name='Collectibles' url={`/internal/collectibles?userId=${authStore.userId}`} icon='icon-nav-charactercustomizer' />
      <a href='/BuildersClub/Upgrade.ashx'><p className={s.upgradeNowButton}>Upgrade Now</p></a>
      <a href='/admin'><p className={s.adminPanelButton}>Admin Panel</p></a>
    </div>
  </div>
}
